...
class ByteStreamReader extends EventTarget {
   ...
   * @returns チャンクを返却する非同期ジェネレーター
   */
  static async *#createChunkGeneratorN(stream: NodeJS.ReadStream): AsyncGenerator<Uint8Array, void, void> {
  ...
  async read(stream: ReadableStream<Uint8Array> | NodeJS.ReadStream, totalByteCount?: number, options: StreamReadingOptions = {}): Promise<Uint8Array> {
    ...
    let acceptSizeMismatch = false;
    if (typeof options.acceptSizeMismatch === "boolean") {
      acceptSizeMismatch = options.acceptSizeMismatch;
    }
    ...
    if (globalThis.ReadableStream && (stream instanceof ReadableStream)) {
      ...
      chunkGenerator = ByteStreamReader.#createChunkGeneratorN(stream as NodeJS.ReadStream);
    }
